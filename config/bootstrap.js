...
module.exports.bootstrap = async function(done) {
  ...
  const deleteFolderRecursive = async path =>  {
    if (fs.existsSync(path)) {
      for (let entry of await afs.readdir(path)) {
        const curPath = path + "./users" + entry;
  ...
  var mike = await User.create({ emailAddress: 'user@user.com', fullName: 'Oleg Zhvankov', password: await sails.helpers.passwords.hashPassword('user') }).fetch();
  ...
  await (async () => {
    const paths = await Promise.all([
      makeDir('./users/admin@example.com'),
      makeDir('./users/rory@example.com'),
      makeDir('./users/raquel@example.com'),
      makeDir('./users/ozhvankov@gmail.com')
    ]);
